...
class ParticleFilter:
    ...
    def update(self, env, u, z, marker_id):
        ...
        for i in range(self.num_particles):
            ...
            loc_particles[i,:] =env.forward(self.particles[i,:],env.sample_noisy_action(u)).ravel()# use noisy input
            z_hat= env.observe(loc_particles[i,:],marker_id)

            loc_weights[i]= env.likelihood(minimized_angle(z_hat-z), self.beta)+1e-300
    ...
    def resample(self, particles, weights):
        ...
        i=0
        for m in range(1,self.num_particles+1):
