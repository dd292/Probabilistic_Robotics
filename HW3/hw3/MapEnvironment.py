...
class MapEnvironment(object):
    ...
    def compute_distance(self, start_config, end_config):
        ...
        return np.sqrt((end_config[1]-start_config[1])**2 + (end_config[0]-start_config[0])**2)
    ...
    def state_validity_checker(self, config):
        ...
        for iter1 in range(n):
            i= config[:,iter1]
            if (i[0]<self.xlimit[0] or i[0]>=self.xlimit[1] or i[1]<self.ylimit[0] or i[1]>=self.ylimit[1] or self.map[int(i[0])][int(i[1])]==1):
                return False
        return True
    ...
    def h(self, config):
        ...
        """
        # TODO: YOUR IMPLEMENTATION HERE
        return self.compute_distance(config,goal)
